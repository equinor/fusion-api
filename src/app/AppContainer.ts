...
export default class AppContainer extends EventEmitter<AppContainerEvents> {
  currentApp: IDistributedState<AppManifest | null>;
  ...
  constructor(
    ...
    this.currentApp = new DistributedState<AppManifest | null>(
      'currentApp',
      null,
      eventHub
    );
  ...
  async setCurrentAppAsync(appKey: string | null): Promise<void> {
    if (!appKey) {
      ...
      this.previousApp = this.currentApp.state;
    ...
    if (!app) {
      const { data: manifest } = await this.fusionClient.getAppManifestAsync(
        appKey
      );
